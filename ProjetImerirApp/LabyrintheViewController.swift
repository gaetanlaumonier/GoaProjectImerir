...
class LabyrintheViewController: UIViewController {
    ...
    func setupSpikesDict() {
        ...
        if let img = self.spikes.image {
            ...
            if let imgList = img.images {
                ...
                spikesImages.append((img: imgList[1], dmg:2))
                
                if playerClass != "Hacker" {
                    spikesImages.append((img: imgList[2], dmg:5))
                    spikesImages.append((img: imgList[3], dmg:5))
                }
                
                spikesImages.append((img: imgList[4], dmg:2))
            }
        }
    ...
    func looseHealth(_ amount: Int) {
        ...
        print("\(amount) points de vie perdus")
    }
    
    func gainHealth(_ amount: Int) {
        print("\(amount) points de vie gagnés")
    }
